...
network *loss_load_network(char *cfg, char *weights, int clear)
{
    network *net = loss_parse_network_cfg(cfg);
    if(weights && weights[0] != 0){
        load_weights(net, weights);
    }
    if(clear) (*net->seen) = 0;
...
float loss_network_datum(network *net)
{
    *net->seen += net->batch;
    //*net->seen += 1;
    net->train = 1;
...
float loss_network(network *net, data d, float *error)
{
    assert(d.X.rows % net->batch == 0);
    int batch = net->batch;
    //int batch = 1;
    ...
    for(i = 0; i < n; ++i){
        printf("-----i-----:%d\n", i);
        get_next_batch(d, batch, i*batch, net->input, net->truth);
        float err = loss_network_datum(net);
        sum += err;

    }
    ;

    //printf("(float)sum/(1):%f\n",(float)sum/(1));
    //printf("(float)sum/(n*batch):%f\n",(float)sum/(n*batch));
    //*error = (float)sum/(n*batch);
    *error = (float)sum;
    //return (float)sum/(1);
    return (float)sum/(n*batch);
}
