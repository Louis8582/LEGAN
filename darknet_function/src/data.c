...
char **get_paths_loss(char **paths, int n, int m)
{
    printf("get_paths_loss\n");
    char **random_paths = calloc(n, sizeof(char*));
    int i;
    int index = 0;
    pthread_mutex_lock(&mutex);
    //int j;
    printf("get_paths_loss n  %d\n",n);
    //printf("get_paths_loss paths[%d]  %s\n",n,paths[n]);
    random_paths[0] = paths[n];
    //for(j = 0;j<n;j++){
        //printf("j %d\n",j);
        //printf("get paths loss paths[j] %s\n",paths[j]);
        //random_paths[j] = paths[j];
    //}
    for(i = 0; i < n; ++i){
        printf("get paths loss paths[i] %s\n",paths[i]);
        random_paths[i] = paths[i];
        if(i == 0) printf("%s\n", paths[index]);
    }
    pthread_mutex_unlock(&mutex);
    return random_paths;
...
data load_data_detection_loss(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
    printf("load_data_detection_loss\n");
    printf("load_data_detection_loss n %d\n",n);
    char **random_paths = get_paths_loss(paths, n, m);
    int i;
    data d = {0};
    d.shallow = 0;
    ...
    for(i = 0; i < n; ++i){
        image orig = load_image_color(random_paths[i], 0, 0);
        printf("load_data_detection_loss i=%d orig.c %d\n",i,orig.c);
        printf("random_paths[%d]:%s\n",i, random_paths[i]);
        image sized = make_image(w, h, orig.c);
        printf("FFFFFFF\n");
        fill_image(sized, .5);
...
void *load_thread_loss(void *ptr)
{
    printf("load_thread_loss\n");
    //printf("load_thread\n");
    //printf("Loading data: %d\n", rand());
    load_args a = *(struct load_args*)ptr;
    if(a.exposure == 0) a.exposure = 1;
    ...
    if (a.type == OLD_CLASSIFICATION_DATA){
        ...
        *a.d = load_data_region(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
        ...
        printf("load_thread_loss n %d\n",a.n);
        *a.d = load_data_detection_loss(a.n, a.paths, a.m, a.w, a.h, a.num_boxes, a.classes, a.jitter, a.hue, a.saturation, a.exposure);
    } else if (a.type == SWAG_DATA){
        *a.d = load_data_swag(a.paths, a.n, a.classes, a.jitter);
    } else if (a.type == COMPARE_DATA){
        *a.d = load_data_compare(a.n, a.paths, a.m, a.classes, a.w, a.h);
...
pthread_t load_data_in_thread_loss(load_args args)
{
    printf("load_data_in_thread_loss\n");
    pthread_t thread;
    struct load_args *ptr = calloc(1, sizeof(struct load_args));
    *ptr = args;
    if(pthread_create(&thread, 0, load_thread_loss, ptr)) error("Thread creation failed");
    return thread;
}
...
void *load_threads_loss(void *ptr)
{
    printf("load_threads_loss\n");
    int i;
    load_args args = *(load_args *)ptr;
    if (args.threads == 0) args.threads = 1;
    data *out = args.d;
    ...
    pthread_t *threads = calloc(args.threads, sizeof(pthread_t));
    for(i = 0; i < args.threads; ++i){
        printf("load_threads_loss i %d\n",i);
        args.d = buffers + i;
        args.n = args.n;
        printf("load threads loss n %d\n",args.n);
        threads[i] = load_data_in_thread_loss(args);
    }
    for(i = 0; i < args.threads; ++i){
        pthread_join(threads[i], 0);
    }
...
pthread_t load_data_loss(load_args args)
{
    printf("load_data_loss\n");
    pthread_t thread;
    struct load_args *ptr = calloc(1, sizeof(struct load_args));
    *ptr = args;
    if(pthread_create(&thread, 0, load_threads_loss, ptr)) error("Thread creation failed");
    return thread;
}
